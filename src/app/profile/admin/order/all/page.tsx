...
import { getOrders, Order, changeOrderStatus } from "@/redux/slices/orderSlice";
import {
  ChevronLeftIcon,
  ChevronRightIcon,
  PencilIcon,
...
const AllOrders = () => {
  ...
  const [newStatus, setNewStatus] = useState<"completed" | "cancelled" | null>(
    null
  );
  const [isModalVisible, setIsModalVisible] = useState(false);
  const { orders, total, totalPages, page, limit, loading } = useSelector(
  ...
  const handleOk = () => {
    if (updateOrderId && newStatus) {
      dispatch(changeOrderStatus({ orderId: updateOrderId, newStatus }))
        .unwrap()
        .then(() => {
  ...
  const getStatusColor = (status: string): string => {
    switch (status.toLowerCase()) {
      case "processing":
        return "text-yellow-600 bg-yellow-50 font-bold";
      case "completed":
    ...
    <div className="container mx-auto px-4 py-8 min-h-screen">
      ...
      >
        <Select
          ...
          onChange={(value: "completed" | "cancelled") => setNewStatus(value)}
          value={newStatus}
        >
                ...
                {orders.map((order: Order) => (
                  ...
                  >
                    ...
                    <td className={`py-3 text-center`}>
                      <p
                        className={` text-center p-1 rounded-full ${getStatusColor(
                          order.status
                        )} `}
                      >
                        {" "}
